...
export const TaskCard: React.FC<TaskCardProps> = ({
  ...
  // Get stage color for the accent border
  const getStageColor = (stage: Stage): string => {
    const colors: Record<Stage, string> = {
      stopped: 'var(--stage-stopped)',
      continue: 'var(--stage-continue)',
      backlog: 'var(--stage-backlog)',
      summarize: 'var(--stage-summarize)',
      create: 'var(--stage-create)',
      plan: 'var(--stage-plan)',
      select: 'var(--stage-select)',
      distribute: 'var(--stage-distribute)',
      work: 'var(--stage-work)',
  ...
  const stageColor = task.errorMessage ? 'var(--accent-danger)' : getStageColor(task.currentStage);
  ...
  return (
    <div
      ...
      onClick={onClick}
      className={`
        task-card p-4 rounded-lg cursor-pointer transition-all duration-200
        hover:brightness-110 hover:translate-y-[-2px] hover:shadow-lg
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="flex items-start gap-2 flex-1">
          ...
          />
          <h4 className="font-semibold text-sm leading-tight line-clamp-2 flex-1" style={{ color: '#ffffff' }}>
            {task.title}
          </h4>
        </div>
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          <button
            ...
            className="p-1 rounded hover:bg-[rgba(239,71,67,0.15)] transition-colors text-muted hover:text-danger"
            title="Delete task"
          >
          ...
          <span
            className={`status-dot mt-1 ${getStatusDotClass(task.status)}`}
            title={task.status}
          />
        </div>
      </div>
      ...
      {/* Description */}
      {task.description && (
        <p className="text-xs line-clamp-2 mb-3" style={{ color: '#b8c0cc' }}>
          {task.description}
        </p>
      )}
